	...
	$t.getSelectedDiv = function(range, editor)
	{
		...
		if(ancestor != editor && (ancestor.nodeName == "DIV" || ancestor.nodeName == "#text"))
		{
			if(sc == ec)
			{
				...
				{
					console.log("일단 여기로 오지 않겠어?");
					if(sc.parentElement.nodeName == "SPAN")
					{
						if(sc.parentElement.parentElement == editor)
						{
							var div = document.createElement("div");
							var childNodes = sc.parentElement.parentElement.childNodes;
						...
						{
							list.push(sc.parentElement.parentElement);
						}
					}
				}
			}
