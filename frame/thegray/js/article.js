...
function bindReplyComment()
{
	...
	$("*[data-id='editComment']").on("click", function()
	{
		...
		if(result.code == 1000)
		{
			var password = "";
			if(!result.id)
				password = '<div><input type="password" name="password" required="required" placeholder="비밀번호를 입력하세요" style="font-size:11px;"/></div>';
	...
	$("*[data-id='deleteComment']").on("click", function()
	{
		...
		{
			var result = $(this).next().get(0).checkValidity();
			if(!result)
			{
		...
		$('<input type="password" required="required" placeholder="비밀번호를 입력하세요" style="font-size:11px; margin:0 3px;"/>').insertAfter(this);
		$(this).next().focus();
	});
}